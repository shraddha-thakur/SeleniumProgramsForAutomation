	...
	public void verifyBrokenLinks(String linkURL) {
		...
		try {
			...
			if (respCode >= 400) {
				...
				ExtentManager.getExtentTest().fail(linkURL + ": is a broken link" + "---" + httpURLConnect.getResponseMessage()+ "---" + httpURLConnect.getResponseCode());
				...
				ExtentManager.getExtentTest().pass(url + ": is a valid link" + "---" + httpURLConnect.getResponseMessage()+ "---" + httpURLConnect.getResponseCode());
